"""Sync MemCore client over the v1 HTTP API (httpx.Client).
...
Sync mirror of :class:`AsyncMemCoreClient` — same surface, blocking I/O. The
parity test (``test_sdk_sync.py``) guards signature drift.
"""
...
class MemCoreClient:
        ...
        while True:
            try:
                ...
                if is_retryable(method, None) and attempt + 1 < self._retry.max_attempts:
                    self._sleep(compute_backoff(attempt, self._retry))
                    attempt += 1
                    continue
    ...
    # -- sessions ----------------------------------------------------------
    def open_session(self, agent_id: str) -> Session:
        data = self._request("POST", "/v1/sessions", json={"agent_id": agent_id})
    ...
    def close_session(self, session_id: str) -> Session:
        data = self._request("POST", f"/v1/sessions/{session_id}/close")
        return Session.model_validate(data["session"])
    ...
    def memory_versions(self, memory_id: str) -> list[MemoryRecord]:
        data = self._request("GET", f"/v1/memories/{memory_id}/versions")
    ...
    def correct_memory(
        ...
        *,
        ...
        if tags is not None:
            body["tags"] = tags
        data = self._request("PATCH", f"/v1/memories/{memory_id}", json=body)
        return MemoryRecord.model_validate(data["memory"])

    def forget_memory(self, memory_id: str, *, mode: str = "soft") -> None:
        self._request(
    ...
    # -- jobs ---------------------------------------------------------------
    def consolidate(self, session_id: str) -> Job:
        data = self._request(
    ...
    def run_decay(self) -> Job:
        data = self._request("POST", "/v1/decay")
        return Job.model_validate(data)
